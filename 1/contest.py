...
def extract_ibm_data(filename, test_file=False):
    ...
    target_position = len(rows[0].split(',')) - 1
    for row in rows:
        cols = row.split(',')
        if not test_file:
...
def train_and_label(train_filename, test_filename, clf):
    ...
    ######
    #Uncomment the below if you later want to use the pickled version (then just change the line above accordingly)
    #pickle.dump(train_data, open("train.p", "wb"))
    #train_data = pickle.load(open("train.p", "rb"))
    # (Could do the same with eval data)
    ########
...
def train_and_single_label(train_filename, test_filename, clf):
...
### Usage: python contest.py train_file test_file {--clf classifier_name}
if __name__ == '__main__':
    ...
    #try this first then substitute it with your version of "train_and_single_label"
    '''*** Modify the following line ***'''
    answers = train_and_label(args.train_file, args.test_file, model)
